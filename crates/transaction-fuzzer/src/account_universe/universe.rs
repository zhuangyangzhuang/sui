...
impl AccountPicker {
    ...
    fn pick_pair(&mut self, indexes: &[Index; PICK_SIZE]) -> [usize; PICK_SIZE] {
        match self {
            AccountPicker::Unlimited(num_accounts) => Self::pick_pair_impl(*num_accounts, indexes),
            AccountPicker::Limited(remaining) => Self::pick_pair_impl(remaining.len(), indexes)
                .map(|idx| {
    ...
    fn pick_pair_impl(max: usize, indexes: &[Index; PICK_SIZE]) -> [usize; PICK_SIZE] {
...
impl AccountPairGen {
    ...
    pub fn pick<'a>(&self, universe: &'a mut AccountUniverse) -> AccountTriple<'a> {
        let [low_idx, mid_idx, high_idx] = universe.picker.pick_pair(&self.pair);
